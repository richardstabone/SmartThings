/**
 *  ****************  Fibaro Colour Switch (Motion Controlled) ****************
 *
 *  Design Usage:
 *  This was designed to be used with the excellent Fibaro RGBW DTH by: 'codersaur' (David Lomas)
 *  With this app you can assign a motion sensor to turn on/off Fibaro colours or programs
 *
 *
...
definition(
    name: "Motion Controlled Fibaro RGBW",
    namespace: "Cobra",
    author: "Andrew Parker",
    description: "Turn On/Off Fibaro RGBW Controller's colour (and brightness) or program with motion sensor",
    category: "My Apps",
...
preferences {
	section(){
        ...
        input "delay", "number", title: "Off delay (Minutes)", required: true
	}
	section(){
    ...
    section(){
     ...
     section("OR.... If Using a 'Custom' colour"){ 
...
state.mydelay = 60 * delay

if(state.motion == 'active'){
picker()
}

else if(state.motion == 'inactive'){
runIn(state.mydelay, switchOff)
}
...
def switchOff() {
	...
	def customRedLevel1 =  0
	def customBlueLevel1 = 0
	def customGreenLevel1 = 0
	def customWhiteLevel1 = 0
